...
PlaySidePiece Bot::selectPiece(Piece piece, PlaySide mySide, bool type) {
    if (!type) {
        if (mySide == BLACK) {
...
PlaySidePiece Bot::switchSide(int x, int y) {
    if (gameBoard[x][y] == BLACK_EN_PAS || gameBoard[x][y] == P_BLACK_ROOK
        || gameBoard[x][y] == P_BLACK_BISHOP
        || gameBoard[x][y] == P_BLACK_KNIGHT
        || gameBoard[x][y] == P_BLACK_QUEEN) {
        return WHITE_PAWN;
    }

    if (gameBoard[x][y] == WHITE_EN_PAS || gameBoard[x][y] == P_WHITE_ROOK
        || gameBoard[x][y] == P_WHITE_BISHOP
        || gameBoard[x][y] == P_WHITE_KNIGHT
...
void Bot::recordMove(Move* move, PlaySide sideToMove) {
    ...
    if (move->isPromotion()) {
        ...
        gameBoard[x_end][y_end]
            = selectPiece(move->getReplacement().value(), sideToMove, false);
        gameBoard[x_start][y_start] = NO_PIECE;
    }
    ...
    if (move->isDropIn()) {
        ...
        gameBoard[x_end][y_end]
            = selectPiece(move->getReplacement().value(), sideToMove, true);
    }
...
std::vector<MoveContext> Bot::moveWhitePawn(int x, int y, PlaySide side) {
    ...
    if (x == 2 && gameBoard[x + 1][y] == NO_PIECE
        && gameBoard[x + 2][y] == NO_PIECE) {
        MoveContext newMove(this,
            Move::moveTo(getPosition(x, y), getPosition(x + 2, y)));
    ...
    if (gameBoard[x + 1][y] == NO_PIECE) {
        MoveContext newMove(this,
            Move::moveTo(getPosition(x, y), getPosition(x + 1, y)));
        newMove.currentBoard[x][y]     = NO_PIECE;
    ...
    if (y + 1 <= TABLE_SIZE && gameBoard[x + 1][y + 1] < NO_PIECE) {
        MoveContext newMove(this,
            Move::moveTo(getPosition(x, y), getPosition(x + 1, y + 1)));
    ...
    if (y - 1 > 0 && gameBoard[x + 1][y] == NO_PIECE
        && gameBoard[x][y - 1] == BLACK_EN_PAS) {
        MoveContext newMove(this,
            Move::moveTo(getPosition(x, y), getPosition(x + 1, y - 1)));
    ...
    if (y + 1 <= TABLE_SIZE && gameBoard[x + 1][y] == NO_PIECE
        && gameBoard[x][y + 1] == BLACK_EN_PAS) {
        MoveContext newMove(this,
            Move::moveTo(getPosition(x, y), getPosition(x + 1, y + 1)));
        newMove.myCapturedPieces.push_back(switchSide(x, y + 1));
...
std::vector<MoveContext> Bot::moveKnight(int x, int y, PlaySide side) {
    ...
    for (int i = 0; i < KNIGHT_MOVES; i++) {
        ...
        MoveContext newMove(this,
            Move::moveTo(getPosition(x, y), getPosition(x + dx[i], y + dy[i])));
...
std::vector<MoveContext> Bot::rook_moves(int x, int y, PlaySide side) {
    ...
    for (int i = x + 1; i <= TABLE_SIZE; i++) {
        if (gameBoard[i][y] != NO_PIECE
            && gameBoard[i][y] * gameBoard[x][y] < 0) {
            MoveContext newMove(this,
                Move::moveTo(getPosition(x, y), getPosition(i, y)));
    ...
    for (int i = y - 1; i > 0; i--) {
        if (gameBoard[x][i] != NO_PIECE
            && gameBoard[x][i] * gameBoard[x][y] < 0) {
            MoveContext newMove(this,
                Move::moveTo(getPosition(x, y), getPosition(x, i)));
    ...
    fout << "ROOK at: " << x << " " << y
         << ", has possible moves: " << moves.size() << std::endl;
...
std::vector<MoveContext> Bot::bishop_moves(int x, int y, PlaySide side) {
    ...
    for (int i = 1; i + x <= TABLE_SIZE && i + y <= TABLE_SIZE;
        ...
        if (gameBoard[x + i][y + i] != NO_PIECE
            ...
            MoveContext newMove(this,
                Move::moveTo(getPosition(x, y), getPosition(x + i, y + i)));
...
std::vector<MoveContext> Bot::queen_moves(int x, int y, PlaySide side) {
    ...
    fout << "QUEEN at: " << x << " " << y
         << ", has possible moves: " << queen_moves.size() << std::endl;
...
std::vector<MoveContext> Bot::createCrazyHouse(int x, int y, PlaySide side) {
    ...
    for (auto& piece : myCapturedPieces) {
        ...
        // Most likely it is not legal to simulate introducing a CrazyHouse
        // piece by movien it from (i, j) to (i, j)
        MoveContext newMove(this,
            Move::dropIn(getPosition(x, y), convertPlaySidePiece(piece)));
...
Move* Bot::calculateNextMove() {
    ...
    for (int i = 1; i <= TABLE_SIZE; i++) {
        for (int j = 1; j <= TABLE_SIZE; j++) {
            ...
            case WHITE_PAWN:
            case WHITE_EN_PAS: {
                std::vector<MoveContext> whitePawnMoves
                    = moveWhitePawn(i, j, engineSide);
                possibleMoves.insert(possibleMoves.end(),
                    whitePawnMoves.begin(), whitePawnMoves.end());
                break;
            }
            case BLACK_PAWN:
            case BLACK_EN_PAS: {
                std::vector<MoveContext> blackPawnMoves
                    = moveBlackPawn(i, j, engineSide);
                possibleMoves.insert(possibleMoves.end(),
                    blackPawnMoves.begin(), blackPawnMoves.end());
                break;
            }
            case WHITE_ROOK:
            case BLACK_ROOK: {
                std::vector<MoveContext> rookMoves
                    = rook_moves(i, j, engineSide);
            ...
            case WHITE_BISHOP:
            case BLACK_BISHOP: {
                std::vector<MoveContext> bishopMoves
                    = bishop_moves(i, j, engineSide);
            ...
            case WHITE_KNIGHT:
            case BLACK_KNIGHT: {
                std::vector<MoveContext> knightMoves
                    = moveKnight(i, j, engineSide);
            ...
            case P_WHITE_QUEEN:
            case P_BLACK_QUEEN: {
                std::vector<MoveContext> queenMoves
                    = queen_moves(i, j, engineSide);
            ...
            default:
                std::vector<MoveContext> crazyMoves
                    = createCrazyHouse(i, j, engineSide);
    ...
    fout << "Total possible moves: " << possibleMoves.size() << std::endl;
...
bool MoveContext::checkPawnAttack(PlaySide side, PlaySidePiece pawn, int& x,
    int& y) {
    if (side == BLACK) {
        /* check for black pawns -- they attack on a lower x value */
        for (int i = 1; i <= TABLE_SIZE; i++) {
